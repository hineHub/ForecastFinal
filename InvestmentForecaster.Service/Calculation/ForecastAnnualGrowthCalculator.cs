...
namespace InvestmentForecaster.Service
{
    ...
    {
        public async Task<IEnumerable<ForecastResponse>> Calculate(IBounds bounds, ForecastRequestDto request)
        {
            ...
            var response = new List<ForecastResponse>() { new ForecastResponse(request.LumpSumInvestment) };
            ...
            foreach (int year in Enumerable.Range(1, request.InvestmentTermInYears))
            {
                ...
                response.Add(new ForecastResponse(year, runningTotal, wideLowerTotal, 
